...
def show_statistics(imatrix_path: str) -> bool:
    ...
    try:
        ...
        # Print header
        print(f"\n{theme['success']}Computing statistics for {imatrix_path} ({len(tstats)} tensors){Style.RESET_ALL}")
        ...
        for ts in tstats:
            ...
            print(f"{layer:>5}  {name:<20}  {ts.total_sqract:>14.2f}  {ts.min_sqract:>10.4f}  {ts.max_sqract:>14.4f}  {ts.mean_sqract:>10.2f}  {ts.stddev:>10.2f}   {ts.active * 100:>7.2f}%  {ts.elements:>10}  {ts.entropy:>12.4f}  {norm_entropy:>7.2f}%  {ts.zd * 100:>10.2f}%  {ts.cossim:>8.4f}")
        ...
        if layers:
            print(f"\n{theme['success']}Computing weighted average statistics per layer ({len(layers)} layers){Style.RESET_ALL}")
