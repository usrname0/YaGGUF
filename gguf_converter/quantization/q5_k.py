"""
Q5_K quantization implementation - simplified version

Q5_K format (from llama.cpp ggml-common.h):
...
- uint8_t qh[32]: quants, high bit (5th bit) (32 bytes)
- uint8_t qs[128]: quants, low 4 bits (128 bytes)
- Total: 176 bytes per super-block

Dequantization formula: x = d * scale * q - dmin * min
where q is 5-bit (0-31)
"""
...
def get_scale_min_k4(j, scales_packed):
    ...
    This is shared with Q4_K - same scale/min packing scheme
...
def quantize_q5_k(tensor, scalar_optimization=False, verbose=False):
    """
    Quantize a float tensor to Q5_K format (simplified from llama.cpp)
    ...
    Returns:
        bytes: quantized data in Q5_K format
    """
        ...
        for i in range(nb):
            ...
            for j in range(8):  # 8 blocks of 32 elements
                ...
                if scalar_optimization:
                    ...
                    # Use grid search optimization (nmax=31 for 5-bit)
                    scale, min_val, L_block = make_qkx3_quants(
                        n=32, nmax=31, x=block, weights=weights,
                    ...
                    # Range needs to map to 0-31 (5-bit)
                    if max_val > min_val:
                        scale = (max_val - min_val) / 31.0  # 31 instead of 15 for 5-bit
                    else:
                        scale = 0.0
            ...
            for j in range(8):
                ...
                l = np.clip(l, 0, 31)  # Clip to 5-bit range (0-31)
                L[32 * j:32 * (j + 1)] = l

            # Step 6: Pack 5-bit values (split into low 4 bits and high 1 bit)
            # qh: high bit (5th bit) - 32 bytes
            # qs: low 4 bits - 128 bytes
            ...
            output.extend(qh.tobytes())  # 32 bytes
            output.extend(qs.tobytes())  # 128 bytes

    return bytes(output)


def dequantize_q5_k(data, n_elements):
    """
    Dequantize Q5_K format back to float32

    Args:
        data: bytes in Q5_K format
    ...
    for i in range(nb):
        ...
        for j in range(0, QK_K, 64):
            ...
            # Unpack and dequantize first 32 elements (lower nibbles + high bit)
            for l in range(32):
                q_low = qs[ql_idx + l] & 0xF
                q_high = 16 if (qh[l] & u1) else 0
                q = q_low + q_high
                output[y_idx + l] = d1 * q - min1

            # Unpack and dequantize second 32 elements (upper nibbles + high bit)
            for l in range(32):
                q_low = qs[ql_idx + l] >> 4
                q_high = 16 if (qh[l] & u2) else 0
                q = q_low + q_high
...
def test_q5_k():
    """Test Q5_K quantization with small tensor"""
    print("Testing Q5_K quantization...")
    ...
    # Quantize
    quantized = quantize_q5_k(test_data)
    expected_size = (512 // QK_K) * 176
    ...
    # Dequantize
    dequantized = dequantize_q5_k(quantized, len(test_data))
    ...
    start = time.time()
    quantized_large = quantize_q5_k(large_data)
    ...
    # Test 3: Scale/min packing roundtrip (reuse from Q4_K)
